...
names = [
    json.loads(redis_connection.lindex(f"patient_{id}", 0))["firstname"]
    for id in range(1, 7)
]
...
def generate_metric_list_header():
    return generate_metric_row(
        "metric_header",
        {"height": "30px", "margin": "10px 0px", "textAlign": "center"},
        {"id": "m_header_1", "children": html.Div("Sensor")},
        {"id": "m_header_3", "children": html.Div("Wykres")},
        {"id": "m_header_5", "children": html.Div("Aktualny nacisk")},
        {"id": "m_header_6", "children": "Anomalia"},
    )
...
def update_small_graphs(n_intervals, current_patient):
    raw_data = redis_connection.lrange(
        f"patient_{PatientsEnum[current_patient]}", 0, 50
    )
    timestamps = [
        datetime.fromtimestamp(json.loads(raw_data_obj)["timestamp"])
        for raw_data_obj in raw_data
